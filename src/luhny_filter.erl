...
do_filter_line( Prefix, PotentialCC, DigitsOnly, [Digit|Tail], MatchLength ) when Digit > 47, Digit < 58 ->
...
is_luhny_checked( DigitsOnly ) when length(DigitsOnly) >= 14, length(DigitsOnly) =< 16 -> lists:foldl( fun( X, S ) -> 
    sum_up_digits( integer_to_list(X) ) + S end, 0, double_every_second_digit( DigitsOnly ) ) rem 10 =:= 0;
is_luhny_checked( _DigitsOnly ) -> false.
